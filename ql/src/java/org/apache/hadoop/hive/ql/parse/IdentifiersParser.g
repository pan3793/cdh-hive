...
intervalExpression
    ...
    KW_INTERVAL? k1=(StringLiteral|Number) q1=intervalQualifiers ->
		^(TOK_FUNCTION Identifier["internal_interval"] BigintLiteral[Integer.toString(((CommonTree)q1.getTree()).token.getType())] $k1)
    |
    KW_INTERVAL? LPAREN k2=expression RPAREN q2=intervalQualifiers ->
		^(TOK_FUNCTION Identifier["internal_interval"] BigintLiteral[Integer.toString(((CommonTree)q2.getTree()).token.getType())] $k2)
    ;
...
precedenceEqualExpressionSingle
    ...
    | (KW_NOT KW_BETWEEN (min=precedenceBitwiseOrExpression) KW_AND (max=precedenceBitwiseOrExpression))
       -> ^(KW_NOT ^(TOK_FUNCTION Identifier["between"] KW_FALSE $left $min $max))
