...
public class DecimalColumnVector extends ColumnVector {
  ...
  public void set(int elementNum, HiveDecimalWritable writable) {
    //TODO: check why we need to do a NULL check here when the java doc above says caller
    //will always do a null check. Specifically, TestOrcFile passes null into this method
    //which causes NPE
    if (writable == null) {
      noNulls = false;
      isNull[elementNum] = true;
    } else {
      HiveDecimal hiveDec = writable.getHiveDecimal(precision, scale);
      if (hiveDec == null) {
        noNulls = false;
        isNull[elementNum] = true;
      } else {
